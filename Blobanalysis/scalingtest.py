...
gs = gridspec.GridSpec(3, 2,				# ratio of grid space (2 plots per collumn, 3 per row)
...
for run in range(length):
	try:
		...
		time1, x_pos1, y_pos1, f_B1, rho_x1, tau_B1, v_r1, v_rmist1, vdmax1, vimax1, Relem1, B01, Lp1,q951, wci1, rho_s1 = np.loadtxt('{0:03d}Emresults_scalingtest.txt'.format(Measurement), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
		time2, x_pos2, y_pos2, f_B2, rho_x2, tau_B2, v_r2, v_rmist2, vdmax2, vimax2, Relem2, B02, Lp2, q952, wci2, rho_s2 = np.loadtxt('{0:03d}Denresults_scalingtest.txt'.format(Measurement), usecols = (3, 4, 5,  6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
		time3, x_pos3, y_pos3, f_B3, rho_x3, tau_B3, v_r3, v_rmist3, vdmax3, vimax3, Relem3, B03, Lp3, q953,wci3, rho_s3 = np.loadtxt('{0:03d}Blockresults_scalingtest.txt'.format(Measurement), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
		...
		time1, x_pos1, y_pos1, f_B1, rho_x1, tau_B1, v_r1, v_rmist1, vdmax1, vimax1, Relem1, B01, Lp1,q951, wci1, rho_s1 = np.loadtxt('{0:03d}Emresults_scalingtest.txt'.format(Measurements[0]), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
		time2, x_pos2, y_pos2, f_B2, rho_x2, tau_B2, v_r2, v_rmist2, vdmax2, vimax2, Relem2, B02, Lp2, q952, wci2, rho_s2 = np.loadtxt('{0:03d}Denresults_scalingtest.txt'.format(Measurements[0]), usecols = (3, 4, 5,  6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
		time3, x_pos3, y_pos3, f_B3, rho_x3, tau_B3, v_r3, v_rmist3, vdmax3, vimax3, Relem3, B03, Lp3, q953,wci3, rho_s3 = np.loadtxt('{0:03d}Blockresults_scalingtest.txt'.format(Measurements[0]), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 21, 22), unpack = True, skiprows=2)
...
#	print('shape of f_B_Mat', np.shape(f_B3m))
	...
	ax1.errorbar(rho_x/rho_s,v_r/cs,xerr = rho_xerr/rho_s, yerr = v_rerr/cs, marker='s',linewidth = 2, color = 'k')
	ax1.errorbar(rho_x/rho_s,vimax/cs,xerr = rho_xerr/rho_s, yerr = vimaxerr/cs, marker='s', linewidth = 2,color = 'r')
	#ax1.plot(rho_x/rho_s,(2*rho_x*Relem[0]/R)**(0.5),linewidth = 2, color = 'b' , marker = 'o', label = 'Garcia: intertial scaling')
	ax1.errorbar(rho_x/rho_s,2*np.log(2)*(rho_s/rho_x)**2*Lp[0]*Relem[0]/R, yerr = 2*np.log(2)*(rho_s/rho_x)**2*Lp[0]*Relemerr[0]/R,linewidth = 2, color = 'g', marker = 'o', label = 'Krasheninnikov: sheath-connected scaling')
	ax1.errorbar(rho_x/rho_s,(1+3)*(rho_s/rho_x)**2*Lp[0]*Relem[0]/R, yerr = (1+3)*(rho_s/rho_x)**2*Lp[0]*Relemerr[0]/R, linewidth = 2, color = 'orange', marker = 'o', label = r'Manz: warm ion sheath-connected scaling ($\tau_i =$ {0:}'.format(tau_i))
	ax1.errorbar(rho_x/rho_s,(2*rho_x/R)**(0.5)*cs/(1+1/((rho_s**2)*Lp[0])*((R/2)**(0.5))*rho_x**(2.5))*Relem[0]/cs, yerr = (2*rho_x/R)**(0.5)*cs/(1+1/((rho_s**2)*Lp[0])*((R/2)**(0.5))*rho_x**(2.5))*Relemerr[0]/cs,linewidth = 2, color = 'b', marker = 'o', label = 'Theiler scaling')

	ax1.set_ylabel(r'blob velocity $v_r/c_s$')
	ax1.get_yaxis().set_label_coords(-0.1,0.5)
	ax1.set_title('Experimental Case at r ={0:.2f}'.format(x[0]))

	ax2.errorbar(rho_x1/rho_s1,v_r1/cs1,xerr = rho_x1err/rho_s1, yerr = v_r1err/cs1, linewidth = 2, marker='s', color = 'k')
	ax2.errorbar(rho_x1/rho_s1,vimax1/cs1,xerr = rho_x1err/rho_s1, yerr = vimax1err/cs1, linewidth = 2, marker='s', color = 'r')
	#ax2.plot(rho_x1/rho_s1,(2*rho_x1*Relem1[0]/R1)**(0.5),linewidth = 2, color = 'b' , marker = 'o', label = 'Garcia: intertial scaling')
	ax2.errorbar(rho_x1/rho_s1,2*np.log(2)*(rho_s1/rho_x1)**2*Lp1[0]*Relem1[0]/R, yerr = 2*np.log(2)*(rho_s1/rho_x1)**2*Lp1[0]*Relem1err[0]/R,linewidth = 2, color = 'g', marker = 'o', label = 'Krasheninnikov: sheath-connected scaling')
	ax2.errorbar(rho_x1/rho_s1,(1+tau_i)*(rho_s1/rho_x1)**2*Lp1[0]*Relem1[0]/R, yerr = (1+tau_i)*(rho_s1/rho_x1)**2*Lp1[0]*Relem1err[0]/R,linewidth = 2, color = 'orange', marker = 'o', label = r'Manz: warm ion sheath-connected scaling ($\tau_i =$ {0:}'.format(tau_i))
	ax2.errorbar(rho_x1/rho_s1,(2*rho_x1/R)**(0.5)*cs1/(1+1/((rho_s1**2)*Lp1[0])*((R/2)**(0.5))*rho_x1**(2.5))*Relem1[0]/cs1, yerr = (2*rho_x1/R)**(0.5)*cs1/(1+1/((rho_s1**2)*Lp1[0])*((R/2)**(0.5))*rho_x1**(2.5))*Relem1err[0]/cs1,linewidth = 2, color = 'b', marker = 'o', label = 'Theiler scaling')


	ax2.set_ylabel(r'blob velocity $v_r/c_s$')
	ax2.get_yaxis().set_label_coords(-0.1,0.5)
	ax2.set_title('Emission Case at r ={0:.2f}'.format(x1[0]))


	ax3.errorbar(rho_x2/rho_s2,v_r2/cs2,xerr = rho_x2err/rho_s2, yerr = v_r2err/cs2, marker='s',linewidth = 2, color = 'k')
	ax3.errorbar(rho_x2/rho_s2,vimax2/cs2,xerr = rho_x2err/rho_s2, yerr = vimax2err/cs2, marker='s', linewidth = 2, color = 'r')
	ax3.errorbar(rho_x2/rho_s2,2*np.log(2)*(rho_s2/rho_x2)**2*Lp2[0]*Relem2[0]/R2, yerr = 2*np.log(2)*(rho_s2/rho_x2)**2*Lp2[0]*Relem2err[0]/R2, linewidth = 2, color = 'g', marker = 'o', label = 'Krasheninnikov: sheath-connected')
	ax3.errorbar(rho_x2/rho_s2,(1+tau_i)*(rho_s2/rho_x2)**2*Lp2[0]*Relem2[0]/R2,yerr = (1+tau_i)*(rho_s2/rho_x2)**2*Lp2[0]*Relem2err[0]/R2, linewidth = 2, color = 'orange', marker = 'o', label = r'Manz: warm ion sheath-connected ($\tau_i =$ {0:})'.format(tau_i))
	ax3.errorbar(rho_x2/rho_s2,(2*rho_x2/R)**(0.5)*cs2/(1+1/((rho_s2**2)*Lp2[0])*((R/2)**(0.5))*rho_x2**(2.5))*Relem2[0]/cs2, yerr = (2*rho_x2/R)**(0.5)*cs2/(1+1/((rho_s2**2)*Lp2[0])*((R/2)**(0.5))*rho_x2**(2.5))*Relem2err[0]/cs2,linewidth = 2, color = 'b', marker = 'o', label = 'Theiler scaling')

	ax3.set_ylabel(r'blob velocity $v_r/c_s$')
	ax3.get_yaxis().set_label_coords(-0.1,0.5)
	ax3.set_title('Density Case at r ={0:.2f}'.format(x2[0]))


	ax4.errorbar(rho_x3/rho_s3,v_r3/cs3,xerr = rho_x3err/rho_s3, yerr = v_r3err/cs3, marker='s',linewidth = 2, color = 'k')
	ax4.errorbar(rho_x3/rho_s3,vimax3/cs3,xerr = rho_x3err/rho_s3, yerr = vimax3err/cs3, marker='s', linewidth = 2,color = 'r')
	ax4.errorbar(rho_x3/rho_s3,2*np.log(2)*(rho_s3/rho_x3)**2*Lp3[0]*Relem3[0]/R3,yerr = 2*np.log(2)*(rho_s3/rho_x3)**2*Lp3[0]*Relem3err[0]/R3,linewidth = 2, color = 'g', marker = 'o', label = 'Krasheninnikov: sheath-connected scaling')
	ax4.errorbar(rho_x3/rho_s3,(1+tau_i)*(rho_s3/rho_x3)**2*Lp3[0]*Relem3[0]/R3, yerr = (1+tau_i)*(rho_s3/rho_x3)**2*Lp3[0]*Relem3err[0]/R3, linewidth = 2, color = 'orange', marker = 'o', label = r'Manz: warm ion sheath-connected scaling ($\tau_i =$ {0:}'.format(tau_i))
	ax4.errorbar(rho_x3/rho_s3,(2*rho_x3/R)**(0.5)*cs3/(1+1/((rho_s3**2)*Lp3[0])*((R/2)**(0.5))*rho_x3**(2.5))*Relem3[0]/cs3, yerr = (2*rho_x3/R)**(0.5)*cs3/(1+1/((rho_s3**2)*Lp3[0])*((R/2)**(0.5))*rho_x3**(2.5))*Relem3err[0]/cs3,linewidth = 2, color = 'b', marker = 'o', label = 'Theiler scaling')

	ax4.set_ylabel(r'blob velocity $v_r/c_s$')
	ax4.set_xlabel(r'blob width $\rho_x/\rho_s$')
	ax4.get_yaxis().set_label_coords(-0.1,0.5)
	ax4.set_title('Block Case at r ={0:.2f}'.format(x3[0]))

	ax5.errorbar(rho_x4/rho_s4,v_r4/cs4,xerr = rho_x4err/rho_s4, yerr = v_r4err/cs4, marker='s', color = 'k', linewidth = 2,label = 'Average velocity data', ls = '')
	ax5.errorbar(rho_x4/rho_s4,vimax4/cs4,xerr = rho_x4err/rho_s4, yerr = vimax4err/cs4, marker='s', color = 'r',linewidth = 2, label = 'Maximum velocity data', ls = '')
	#ax5.plot(rho_x4/rho_s4,(2*rho_x4*Relem4[0]/R4)**(0.5),linewidth = 2, color = 'b' , marker = 'o', label = 'Garcia: intertial scaling')
	ax5.errorbar(rho_x4/rho_s4,2*np.log(2)*(rho_s4/rho_x4)**2*Lp4[0]*Relem4[0]/R4,yerr = 2*np.log(2)*(rho_s4/rho_x4)**2*Lp4[0]*Relem4err[0]/R4, color = 'g',linewidth = 2, marker = 'o', label = 'Krasheninnikov: sheath-connected', ls = '')
	ax5.errorbar(rho_x4/rho_s4,(1+tau_i)*(rho_s4/rho_x4)**2*Lp4[0]*Relem4[0]/R4,yerr = (1+tau_i)*(rho_s4/rho_x4)**2*Lp4[0]*Relem4err[0]/R4,  color = 'orange', linewidth = 2,marker = 'o', label = r'Manz: warm ion sheath-connected ($\tau_i =$ {0:})'.format(tau_i), ls = '')
	ax5.errorbar(rho_x4/rho_s4,(2*rho_x4/R)**(0.5)*cs4/(1+1/((rho_s4**2)*Lp4[0])*((R/2)**(0.5))*rho_x4**(2.5))*Relem4[0]/cs4, yerr = (2*rho_x4/R)**(0.5)*cs4/(1+1/((rho_s4**2)*Lp4[0])*((R/2)**(0.5))*rho_x4**(2.5))*Relem4err[0]/cs4, color = 'b', marker = 'o',linewidth = 2, label = 'Theiler scaling', ls = '')

	handles, labels = ax5.get_legend_handles_labels()
	handles = [h[0] for h in handles]

	ax5.set_ylabel(r'blob velocity $v_r/c_s$')
	ax5.set_xlabel(r'blob width $\rho_x/\rho_s$')
	ax5.get_yaxis().set_label_coords(-0.1,0.5)
	ax5.set_title('Block Case with real SNR at r ={0:.2f}'.format(x4[0]))
	if run == 0:
		leg = ax5.legend(handles, labels,loc='center left', bbox_to_anchor = (1.18,0.5), fancybox = True, numpoints = 1)
