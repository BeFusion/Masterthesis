...
#Case = 0 	#(if specific case is only needed)
	...
	if Radial:	
		...
		if EmCase and NewData:
			fu = open('{0:03d}_speedtest_Emresults.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')
			print('results are stored in {0:03d}_speedtest_Emresults.txt'.format(Measurement))
		if DenCase and NewData:
			fu = open('{0:03d}_speedtest_Denresults.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')
			print('results are stored in {0:03d}_speedtest_Denresults.txt'.format(Measurement))
		if BlockCase and NewData:
			fu = open('{0:03d}_speedtest_Blockresults.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')	
			print('results are stored in {0:03d}_speedtest_Blockresults.txt'.format(Measurement))
	...
	#			plt.switch_backend('Agg')
			...
			# 	Calculate center of mass (COM) for blobs at different t in TWind ######################################################################
				...
				for l in range (0,len(LiConAv[:,1])):
					for k in range (0,len(LiConAv[1,:])):
						if (LiConAv[l,k]<0):
							LiConAv[l,k]=0
				COM = [np.NaN]*len(LiConAv[:,1])		# create COM-vector with appropriate shape
				SPEEDTEST = [np.NaN]*len(LiConAv[:,1])
				for m in range (0,int(TWind/avt)):
					...
					SPEED_inter = max(LiConAv[m,:])
					for l in range (len(LiConAv[1,:])):

						if (SPEED_inter == (LiConAv[m,l])):
							SPEED_ind = l
					if (Blub<0):
			...
			#	 new time window for speedtest:
				for m in range (int(TWind/avt)):
					if (Delt[m] == 0.0):				# look only at positive values for speed calculation
						LookWinMin_ind = m			# new minium of time window 
				
				SPEED_TIME = [np.NaN]*len(LiConAv[:,1])			# stores the maximum value of emission at one position
				SPEED_STORE = [np.NaN]*len(LiConAv[1,:])		# stores the index of the maximum value of emisson at that postion
				MAX_EM = max(LiConAv[LookWinMin_ind,:])
				for m in range (LookWinMin_ind,int(TWind/avt)):
					SPEED_inter = max(LiConAv[m,:])
					SPEED_TIME[m]= SPEED_inter
	#				for l in range (len(LiConAv[1,:])):
	#					if (SPEED_inter == (LiConAv[m,l])):
	#						SPEED_STORE[m] = l
			#		pdb.set_trace()
					if (SPEED_TIME[m]> MAX_EM):
						MAX_EM = SPEED_TIME[m]

				for m in range (LookWinMin_ind, int(TWind/avt)):
					if (SPEED_TIME[m]<=MAX_EM/2):		# if the position has a smaller maximum value than 1/2 of overall maximum
						LookWinMax_ind = m
						print('I am here and exiting this loop now')
						break
				LookWinMin = Delt[LookWinMin_ind]
				LookWinMax = Delt[LookWinMax_ind]
				if LookWinMax_ind <= LookWinMin_ind+5:
					LookWinMax=Delt[LookWinMin_ind+5]
					LookWinMax_ind = LookWinMin_ind + 5
				print('New Time windows for speedtest is from {0:.4f} to {1:.4f}'.format(LookWinMin, LookWinMax))
				...
				v_r_spe, v_intercept_spe, r_value_spe,p_value_spe,std_err_spe = stats.linregress(Delt[LookWinMin_ind:LookWinMax_ind],SPEEDTEST[LookWinMin_ind:LookWinMax_ind])
				...
				(a_spe,b_spe) =polyfit(Delt[LookWinMin_ind:LookWinMax_ind],SPEEDTEST[LookWinMin_ind:LookWinMax_ind],1) 
				SPEED_lgr = np.polyval([a_spe,b_spe],Delt[LookWinMin_ind:LookWinMax_ind])
				...
				XC_SPEED_smooth = spline(Delt,SPEEDTEST,TWind_inter)
				...
				for q in range (LookWinMin_ind+1,LookWinMax_ind):			# Calculate slopes between every point
					print('LookWinMin_ind', 'LookWinMax_ind', 'q', 'len(COM)', 'len(slope_data)', 'len(Delt)')
					print(LookWinMin_ind, LookWinMax_ind, q, len(COM), len(slope_data), len(Delt))  
				...
				for p in range (0,len(TWind_inter)-1):					# Calculate slopes between every point
					slope_inter[p-LookWinMin_ind] = (XC_COM_smooth[p]-XC_COM_smooth[p-1])/abs(TWind_inter[p]-TWind_inter[p-1])
					if (slope_inter[p-LookWinMin_ind]<0):
						slope_inter[p-LookWinMin_ind]=0
				v_imax=np.amax(slope_inter)						# find maximum slope interpolation

				slope_data_spe=[np.NaN]*(LookWinMax_ind-LookWinMin_ind)
				slope_inter_spe=[np.NaN]*(len(TWind_inter)-1)
				slope_data_spe=np.array(slope_data_spe)						# convert list to array
				slope_inter_spe=np.array(slope_inter_spe)					# convert list to array
				slope_data_spe[0]=0
				slope_inter_spe[0]=0
				for q in range (LookWinMin_ind+1,LookWinMax_ind):			# Calculate slopes between every point
					slope_data_spe[q-LookWinMin_ind] = (SPEEDTEST[q]-SPEEDTEST[q-1])/abs(Delt[q]-Delt[q-1])
					if (slope_data_spe[q-LookWinMin_ind]<0):
						slope_data_spe[q-LookWinMin_ind]=0

				v_dmax_spe=np.amax(slope_data_spe)						# find maximum slope of data 
				for p in range (0,len(TWind_inter)-1):					# Calculate slopes between every point
					slope_inter_spe[p-LookWinMin_ind] = (XC_SPEED_smooth[p]-XC_SPEED_smooth[p-1])/abs(TWind_inter[p]-TWind_inter[p-1])
					if (slope_inter_spe[p-LookWinMin_ind]<0):
						slope_inter_spe[p-LookWinMin_ind]=0
				v_imax_spe=np.amax(slope_inter_spe)						# find maximum slope interpolation
				...
				vr_spe = v_r_spe*0.01*pow(10,3)
				rval_spe = r_value_spe*0.01*pow(10,3)
				vdmax_spe = v_dmax_spe*0.01*pow(10,3)
				vimax_spe = v_imax_spe*0.01*pow(10,3)
				...
				print('Average speed of blob (SPEEDTEST):{0:.1f}m/s with standard deviation of {1:.1f}m/s'.format(vr_spe,rval_spe))	# Average speed [m/s]
				print('Maximum speed of blob (real data) (SPEEDTEST): {0:.1f}m/s'.format(vdmax_spe))	# Maximum speed from data [m/s]
				print('Maximum speed of blob (smoothed) (SPEEDTEST): {0:.1f}m/s:'.format(vimax_spe))	# Maximum speed from smoothing [m/s]
			...
			# 	AX23: plot different data for blob position and speed
				...
				XC = ax23.plot(Delt,COM, color = 'k', label = 'Data COM')				# plot original data
				XC_inter = ax23.plot(TWind_inter, XC_COM_smooth, color = 'm', label = 'smoothed COM')	# plot interpolated data
				XC_lgr = ax23.plot(Delt[LookWinMin_ind:LookWinMax_ind],COM_lgr, 'r--', label = 'lin. regresion COM') # plot regression
				XC2 = ax23.plot(Delt,SPEEDTEST, color = 'b', label ='Data Max. position')				# plot original data
				XC_inter2 = ax23.plot(TWind_inter, XC_SPEED_smooth, color = 'g', label = 'scmoothed max. position')	# plot interpolated data
				XC_lgr2 = ax23.plot(Delt[LookWinMin_ind:LookWinMax_ind],SPEED_lgr, 'g--', label = 'lin. regression max. position') # plot regression

				leg = ax23.legend(loc='best', fancybox = True, numpoints = 1)
			...
			if NewData:					# Write only, if needed
				...
				if not BlockCase:
					data = [("00"+str(Measurement), ""), (t_start,".2f"), (t_end-t_start, ".2f"), (x[Refdec_ind], ".2f"), (y[shift*2],".2f"), (BlobCount, "d"), (Delx, ".3f"), (tau_B,".4f"), (Blobf,".1f"), (vr, ".1f"), (rval,".1f"), (vdmax,".1f"), (vimax,".1f"), (B0,".2f"), (Lp,".2f"), (q95,".2f"), (Te0,".2f"), (Ti0,".2f"), (ne0,".2e"), (omegaCi,".2e"), (rhoS,".6f"), (endtime,".2f"), (vr_spe, ".1f"), (rval_spe,".1f"), (vdmax_spe,".1f"), (vimax_spe,".1f")]
				if BlockCase:
					data = [("00"+str(Measurement), ""), (t_start,".2f"), (t_end-t_start, ".2f"), (x[Refdec_ind], ".2f"), (y[shift_Block*2],".2f"), (BlobCount, "d"), (Delx, ".3f"), (tau_B,".4f"), (Blobf,".1f"), (vr, ".1f"), (rval,".1f"), (vdmax,".1f"), (vimax,".1f"), (B0,".2f"), (Lp,".2f"), (q95,".2f"), (Te0,".2f"), (Ti0,".2f"), (ne0,".2e"), (omegaCi,".2e"), (rhoS,".6f"), (endtime,".2f"), (vr_spe, ".1f"), (rval_spe,".1f"), (vdmax_spe,".1f"), (vimax_spe,".1f")]
