...
if not NoiseSmooth:
	filename = '_veltwind'
Measurement = 112		# specify number of measurement without leading zeros, which is the prefix of the file in form (will be extended later to e.g. 004) 
savenow = True
comp = False	#comparison of threshold
comp2 = False 	# comparison of different SNR
...
def radial_analysis(filename,Measurement,Refdec_ind, Refdec_ind3, savenow, SNR, NoiseSmooth, fourCase):
	...
	if comp:
		fourCase = True
		Casei = 'Den' # Alternatives are Em, Den or BLock
	...
	if comp2:
		...
		time, x_pos, y_pos, f_B, rho_x, tau_B, v_r, v_rmist, vdmax, vimax, Relem, B0, Lp,q95 = np.loadtxt('{0:03d}{1:}results{2:}_TIME_SNR_Comparison.txt'.format(Measurement,Casei,filename), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17), unpack = True, skiprows=2)
		time2, x_pos2,y_pos2, f_B2, rho_x2, tau_B2, v_r2, v_rmist2, vdmax2, vimax2, Relem2, B02, Lp2, q952 = np.loadtxt('{0:03d}{1:}results{2:}SNR001.txt'.format(Measurement,Casei,filename), usecols = (3, 4, 5,  6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17), unpack = True, skiprows=2)
		time3, x_pos3, y_pos3, f_B3, rho_x3, tau_B3, v_r3, v_rmist3, vdmax3, vimax3, Relem3, B03, Lp3, q953 = np.loadtxt('{0:03d}{1:}results{2:}SNR004.txt'.format(Measurement,Casei,filename), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17), unpack = True, skiprows=2)
		time4, x_pos4, y_pos4, f_B4, rho_x4, tau_B4, v_r4, v_rmist4, vdmax4, vimax4, Relem4, B04, Lp4, q954 = np.loadtxt('{0:03d}{1:}results{2:}SNR008.txt'.format(Measurement,Casei,filename), usecols = (3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17), unpack = True, skiprows=2)
...
#	print('shape of f_B_Mat', np.shape(f_B3m))
	...
	ax2 = plt.subplot(gs[1])	# vr
	ax3 = plt.subplot(gs[2])	# vdmax
	ax4 = plt.subplot(gs[3])	# tauB
	ax5 = plt.subplot(gs[4])	# vmax
	ax6 = plt.subplot(gs[5])

	# create a list in for simple modifications on all plots at the same time:
	allax=[ax1,ax2,ax3,ax4,ax5, ax6]
	font = {'family' :'Arial', 'weight' : 'normal', 'size' : 29}
	...
	ax1.errorbar(x,rho_x,yerr = rho_xerr, marker='s', color = 'k', linewidth = lwid)
	ax1.errorbar(x2,rho_x2,yerr = rho_x2err,marker='o', color = 'b', linewidth = lwid)
	ax1.errorbar(x3,rho_x3,yerr = rho_x3err,marker='D', color = 'g', linewidth = lwid)	
	if fourCase:
		ax1.errorbar(x4,rho_x4,yerr = rho_x4err,marker='v', color = 'orange', linewidth = lwid)
	ax1.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	#ax1.set_xlabel('beam axis $x$ (cm)', labelpad= -10)			# switched of, since axis in row below
	ax1.set_ylabel(r'$\rho_x$ (cm)')
	ax1.get_yaxis().set_label_coords(-0.1,0.5)
	ax1.set_title(r'Blob width $\rho_x$', y = 1.03)
	...
	#ax1.text(-0.4,2.1, 'LCFS', font)
	ax1.annotate("a)", xycoords="axes fraction", xy = (xshift,yshift), va = 'center', ha = 'center', fontsize = 25)


	v_rfig = ax2.errorbar(x,v_r/np.float64(1000), yerr = v_rerr/np.float64(1000),marker='s', color = 'k', linewidth = lwid)
	v_rfig2 = ax2.errorbar(x2,v_r2/np.float64(1000), yerr = v_r2err/np.float64(1000),marker='o', color = 'b', linewidth = lwid)
	v_rfig3 = ax2.errorbar(x3,v_r3/np.float64(1000), yerr = v_r3err/np.float64(1000),marker='s', color = 'g', linewidth = lwid)
	if fourCase:
		v_rfig4 = ax2.errorbar(x4,v_r4/np.float64(1000), yerr = v_r4err/np.float64(1000),marker='v', color = 'orange', linewidth = lwid)
	ax2.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	...
	ax2.set_title(r'Average velocity $v_r$', y = 1.03)
	ax2.annotate("b)", xycoords="axes fraction", xy = (xshift,yshift), va = 'center', ha = 'center', fontsize = 25)


	f_B2fig = ax3.errorbar(x,f_B/time[10],yerr = f_Berr/time[10], marker='s', color = 'k', linewidth = lwid)
	f_B22fig = ax3.errorbar(x2,f_B2/time[10],yerr = f_B2err/time[10], marker='o', color = 'b', linewidth = lwid)
	f_B23fig = ax3.errorbar(x3,f_B3/time[10],yerr = f_B3err/time[10], marker='D', color = 'g', linewidth = lwid)
	if fourCase:
		f_B23fig = ax3.errorbar(x4,f_B4/time[10],yerr = f_B4err/time[10], marker='v', color = 'orange', linewidth = lwid)
	ax3.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	#ax3.set_xlabel('beam axis $x$ (cm)', labelpad= 0)			# switched of, since axis in row below
	ax3.set_ylabel('$f_B$ (1000/s)')
	ax3.get_yaxis().set_label_coords(-0.1,0.5)
	ax3.set_title('Blob frequency $f_B$', y = 1.03)
	if not comp and not comp2:
		ax3.set_ylim(0,7.0)
	if comp and comp2:
		ax3.set_ylim(0,3.0)
	...
	ax4.errorbar(x,vdmax/np.float64(1000), yerr = vdmaxerr/np.float64(1000),color =  'k',marker='s',label='Emission data', linewidth = lwid)
	ax4.errorbar(x2,vdmax2/np.float64(1000), yerr = vdmax2err/np.float64(1000), color = 'b',marker='o',label='Density data', linewidth = lwid)
	ax4.errorbar(x3,vdmax3/np.float64(1000), yerr = vdmax3err/np.float64(1000), color = 'g',marker='D',label='Block data', linewidth = lwid)
	if fourCase:
		ax4.errorbar(x4,vdmax4/np.float64(1000), yerr =vdmax4err/np.float64(1000), color = 'orange',marker='v',label='Real SNR Block data', linewidth = lwid)
	ax4.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	#ax4.set_xlabel('beam axis $x$ (cm)', labelpad= 0)			# switched of, since axis in row below
	ax4.set_ylabel(r'$v_{max}$ (km/s)')
	ax4.get_yaxis().set_label_coords(-0.11,0.5)
	if not comp and not comp2:
		ax4.set_ylim(0,3.0)
	if comp and comp2:
		ax4.set_ylim(0,1.0)
	ax4.set_title(r'Maximum blob velocity $v_{max}$', y = 1.03)
	ax4.annotate("d)", xycoords="axes fraction", xy = (xshift,yshift), va = 'center', ha = 'center', fontsize = 25)

	if not comp and not comp2:
		ax5.errorbar(x,Relem, yerr = Relemerr,color =  'k',marker='s',label='Emission data', linewidth = lwid)
		ax5.errorbar(x2,Relem2, yerr = Relem2err, color = 'b',marker='o',label='Density data', linewidth = lwid)
		ax5.errorbar(x3,Relem3, yerr = Relem3err, color = 'g',marker='D',label='Block data', linewidth = lwid)
		if fourCase:
			ax5.errorbar(x4,Relem4, yerr = Relem4err, color = 'orange',marker='v',label='Real SNR Block data', linewidth = lwid)
	ax5.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	ax5.set_xlabel('axis $x$ (cm)', labelpad= 0)			# switched of, since axis in row below
	ax5.set_ylabel(r'$\delta I/I$ or $\delta n/n$')
	ax5.get_yaxis().set_label_coords(-0.11,0.5)
	ax5.set_title(r'Blob amplitude $\delta I/I$ or $\delta n/n$', y = 1.03)
	ax5.set_ylim(0,2.5)
	ax5.annotate("e)", xycoords="axes fraction", xy = (xshift,yshift), va = 'center', ha = 'center', fontsize = 25)
	if comp:
		ax5.errorbar(x,Relem, yerr = Relemerr,color =  'k',marker='s',label= r'Run 111, $\sigma$ varied in [1,2]', linewidth = lwid)
		ax5.errorbar(x2,Relem2, yerr = Relem2err, color = 'b',marker='o',label= r'Run 111, $\sigma$ = 0.5', linewidth = lwid)
		ax5.errorbar(x3,Relem3, yerr = Relem3err, color = 'g',marker='D', label= r'Run 111, $\sigma$ = 1', linewidth = lwid)
		if fourCase:
			ax5.errorbar(x4,Relem4, yerr = Relem4err, color = 'orange',marker='v',label= r'Run 111, $\sigma$ = 2', linewidth = lwid)

	if comp2:
		ax5.errorbar(x,Relem, yerr = Relemerr,color =  'k',marker='s',label= r'Real SNR', linewidth = lwid)
		ax5.errorbar(x2,Relem2, yerr = Relem2err, color = 'b',marker='o',label= r'SNR = 1', linewidth = lwid)
		ax5.errorbar(x3,Relem3, yerr = Relem3err, color = 'g',marker='D', label= r'SNR = 4', linewidth = lwid)
		if fourCase:
			ax5.errorbar(x4,Relem4, yerr = Relem4err, color = 'orange',marker='v',label= r'SNR = 8', linewidth = lwid)
	
	ax6.errorbar(x,tau_B/np.float64(0.001), yerr = tau_Berr/np.float64(0.001),color =  'k',marker='s',label='Emission data', linewidth = lwid)
	ax6.errorbar(x2,tau_B2/np.float64(0.001), yerr = tau_B2err/np.float64(0.001), color = 'b',marker='o',label='Density data', linewidth = lwid)
	ax6.errorbar(x3,tau_B3/np.float64(0.001), yerr = tau_B3err/np.float64(0.001), color = 'g',marker='D',label='Block data', linewidth = lwid)
	if fourCase:
		ax6.errorbar(x4,tau_B4/np.float64(0.001), yerr = tau_B4err/np.float64(0.001), color = 'orange',marker='v',label='Real SNR Block data', linewidth = lwid)
	ax6.axvline(0,color='k', linestyle='-.', linewidth = lwid)
	ax6.set_xlabel('axis $x$ (cm)', labelpad= 0)			# switched of, since axis in row below
	ax6.set_ylabel(r'$\tau_B$ ($\mu$s)')
	ax6.get_yaxis().set_label_coords(-0.11,0.5)
	ax6.set_title(r'Self-correlation time $\tau$', y = 1.03)
	#ax6.set_ylim(0,2.5)
	ax6.annotate("f)", xycoords="axes fraction", xy = (xshift,yshift), va = 'center', ha = 'center', fontsize = 25)
	plt.tight_layout()
	...
	leg = ax5.legend(loc='upper center', bbox_to_anchor = (1.1,-0.25),fancybox = True, numpoints = 1, ncol = 2)

	if savenow:
		if NoiseSmooth and not fourCase and not comp and not comp2:
			f1.savefig('{0:03d}FigRadial{1:}'.format(Measurement,filename))
		if not NoiseSmooth and not fourCase and not comp and not comp2:
			f1.savefig('{0:03d}FigRadial{1:}'.format(Measurement,filename))
		if fourCase and not comp and not comp2:
			f1.savefig('{0:03d}FigRadial{1:}_all_cases'.format(Measurement,filename),bbox_extra_artists=(leg,), bbox_inches='tight')
		if comp and savenow:
			f1.savefig('{0:03d}FigRadial_threshold_{1:}results{2:}'.format(Measurement,Casei,Exten),bbox_extra_artists=(leg,), bbox_inches='tight')
		if comp2 and savenow:
			f1.savefig('ü{0:03d}FigRadial_SNRcomp_{1:}results'.format(Measurement,Casei),bbox_extra_artists=(leg,), bbox_inches='tight')
	if not comp and not comp2:
