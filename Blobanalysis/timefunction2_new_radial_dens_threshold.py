...
for Case in range (2,3):
	...
	if Radial:	
		...
		if EmCase and NewData:
			fu = open('{0:03d}Emresults_dens_threshold.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')
			print('results are stored in {0:03d}Emresults.txt'.format(Measurement))
		if DenCase and NewData:
			fu = open('{0:03d}Denresults_dens_threshold.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')
			print('results are stored in {0:03d}Denresults.txt'.format(Measurement))
		if BlockCase and NewData:
			fu = open('{0:03d}Blockresults_dens_threshold.txt'.format(Measurement), 'wb')		# use only writing (option 'w') if needed (ATTENTION: discards the whole file, appending uses 'a')	
	...
	#for dens-threshold case:
	LiBlobEmden = ne.copy()				 # copy Li-Matrix for BlobCount use
	...
	for bcenter in beamrange:
		...
		#	Li2p1 = block_func(Li2p1,timestep,mn,b,avy,shift,shift2)			# if original block-function should be used
			...
			mn = len(x)
			shift_Block = shift
			shift = 0									# we are in the 1D-Case now (Blocks!), so every time the entry at shift is called in matrix, this is set to the zero entry
...
#			Li2p1Z = Li2p1.copy()				 # copy matrix for use in calculation of Lip1Z (the zero-mean-fluctuation)
		...
		if Radial:
			...
			for Refdec_ind in range(Startrange, len(x),Resolution):			# Wall-region can be neglected (approximately 1.5-2cm) --> go through Refdec-Positions
				...
				#for dens-threshold case:
				if BlockCase:
#					Startrangedens = int(1.5/0.02)
#					Refdec_indden = Startrangedens + (Refdec_ind-Startrangedens)*SetResolution
					for i in range(0,len(x_ori)):
						if (x[Refdec_ind]<x_ori[i] and x[Refdec_ind]>x_ori[i+2]):
							Refdec_indden = i
							break
							
				if not BlockCase:
					Refdec_indden = Refdec_ind
				...
				LiAvden = np.mean(ne[int(t_start/avt):int(t_end/avt),shiftden*2,Refdec_indden])			# Average of Li value
				LiSigden = np.std(ne[int(t_start/avt):int(t_end/avt),shiftden*2,Refdec_indden])			# Standard deviation of Li value

				LiCon = LiAv + ThresCon*LiSig									# Set condition
				LiConden = LiAvden + ThresCon*LiSigden
				TWind = TWindMult*avt							# Time window for conditional averaging [us] (Gregor's Publication: 500 us)						# Time window for conditional averaging [us] (Gregor's Publication: 500 us)
...
#					LiBlobEm = Li2p1.copy()						# copy Li-Matrix for BlobCount use
				...
				for m in range (int(t_start/avt),int(t_end/avt)):					# go through every time point
					if LiBlobEmden[m,shiftden*2,Refdec_indden]<LiConden:		# go in, if emission is below threshold
						LiBlobEmden[m,shiftden*2,Refdec_indden]=0		# set all values to zero, if it is below the threshold
					else:							# go in, if emission is above threshold
						LiBlobEmden[m,shiftden*2,Refdec_indden]=1		# set all values to 1, if it is above threshold
					m=m+1
				...
				for m in range (int(t_start/avt),int(t_end/avt)-1):					# go through every time point
					if LiBlobEmden[m,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m+1,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m-1,shiftden*2,Refdec_indden]==0: # condition for counting an event as a blob (0 1 1)
						...
						remind[BlobCount]=m
					if LiBlobEmden[m,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m+1,shiftden*2,Refdec_indden]==0 and LiBlobEmden[m-1,shiftden*2,Refdec_indden]==0:
						BlobCount=BlobCount+1							# raise blob count
						if (remind[BlobCount-1]>=m-WinBar):					# safty time intervall to not count one blob as two
							BlobCount=BlobCount-1
						remind[BlobCount]=m
				...
				# 	Find emission values for standard blobs ###################################################################################
					...
					for m in range (int(t_start/avt),int(t_end/avt)-1-int(TWind/avt)):					# go through every time point until one TWind before the edge --> a possible blob at the right edge-blob is not used
						if LiBlobEmden[m,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m+1,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m-1,shiftden*2,Refdec_indden]==0 or \
						LiBlobEmden[m,shiftden*2,Refdec_indden]>0 and LiBlobEmden[m+1,shiftden*2,Refdec_indden]==0 and LiBlobEmden[m-1,shiftden*2,Refdec_indden]==0: # condition for counting an event as a blob (0 1 1) or (0 1 0)
							Count2=Count2+1
